...
latest_stigma_dogs <- init_latest_stigma[is.na(init_latest_stigma$`T/C Find`), c("Breed Group", "Rescue_ID", "Name - AdoptAPet")]
...
latest_nonstigma_dogs <- init_latest_nonstigma[is.na(init_latest_nonstigma$`T/C Find`), c("Breed Group", "Rescue_ID", "Name - AdoptAPet")]
...
```{r}
set.seed(1013)
total_sitgma[, treatment_10_20_25 := rbinom(.N, 1, 0.5)]
total_sitgma

total_sitgma %>%
  group_by(treatment_10_20_25) %>%
  summarise(count = n())

total_nonsitgma[, treatment_10_20_25 := rbinom(.N, 1, 0.5)]
total_nonsitgma
...
```{r}
write.csv(combined_dt, file = "Randomize_10_20_25.csv", row.names = FALSE)
```

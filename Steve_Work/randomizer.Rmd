...
```{r}
path <- "Experiment_Data .xlsx"
initial_data <- read_excel(path, sheet = "InitialRandom_10_07_25")
...
latest_stigma_dogs <- init_latest_stigma[is.na(init_latest_stigma$`Treatment/Control`), c("Breed Group", "Rescue_ID", "Name - AdoptAPet")]
latest_stigma_dogs
init_latest_nonstigma <- as.data.table(latest_data[latest_data$"Breed Group"=="Non-Stigma",])
latest_nonstigma_dogs <- init_latest_nonstigma[is.na(init_latest_nonstigma$`Treatment/Control`), c("Breed Group", "Rescue_ID", "Name - AdoptAPet")]
...
```{r}
set.seed(1013)
total_sitgma[, treatment_10_18_25 := rbinom(.N, 1, 0.5)]
total_sitgma

total_sitgma %>%
  group_by(treatment_10_18_25) %>%
  summarise(count = n())

total_nonsitgma[, treatment_10_18_25 := rbinom(.N, 1, 0.5)]
total_nonsitgma
...
```{r}
write.csv(combined_dt, file = "Randomize_10_18_25.csv", row.names = FALSE)
```
